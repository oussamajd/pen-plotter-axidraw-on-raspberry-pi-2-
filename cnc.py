...
MX=Bipolar_Stepper_Motor(10,9,11,25);     #pin number for a1,a2,b1,b2.  a1 and a2 form coil A; b1 and b2 form coil B

MY=Bipolar_Stepper_Motor(17,22,18,4);
stilo=12;
dx=0.15; #resolution in x direction. Unit: mm
dy=0.15; #resolution in y direction. Unit: mm
write_speed=10; #unit=mm/sec=0.04in/sec
...
GPIO.setup(stilo,GPIO.OUT);

GPIO.output(stilo,False);

speed=write_speed/min(dx,dy);      #step/sec
...
def stilon():
	global write
        if (write==True):
		write = False
                duty1 = float(angle2)/10 + ajoutAngle
                print "on"
...
def stiloff():
        global write
        duty2= float(angle1)/10+ ajoutAngle
        print "off"
...
def moveto(MX,x_pos,dx,MY,y_pos,dy,speed,write):
#Move to (x_pos,y_pos) (in real unit)
    ...
    if Total_step>0:
        if write == False:#fast movement
                print 'No stilo, fast movement: Dx=', stepx, '  Dy=', stepy;
                Motor_control.Motor_Step(MX,stepx,MY,stepy,100);
        else:
                print 'stilo on, movement: Dx=', stepx, '  Dy=', stepy;
...
try:#read and execute G code
            ...
            if lines==[]:
                ...
                print 'start';
                    ...
                    if (lines.find('X') != -1 and lines.find('Y') != -1):
                            ...
                            moveto(MX,x_pos,dx,MY,y_pos,dy,speed,write);    
                ...
                stiloff()
                print 'stiloturned off';

            elif lines[0:3]=='M03':
                stilon()
                print 'stiloturned on';
                ...
                1;#do nothing
            elif (lines[0:3]=='G1 ')|(lines[0:3]=='G01'):#|(lines[0:3]=='G02')|(lines[0:3]=='G03'):
                if (lines.find('X') != -1 and lines.find('Y') != -1): #Ignore lines not dealing with XY plane
                    #linear write movement
                    stilon()
                    print lines
    ...
    pass
stiloff()   # turn off stilo
moveto(MX,0,dx,MY,0,dy,100,False);  # move back to Origin
